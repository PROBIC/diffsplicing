MH <-
function(mcmc_filenames_in,noLines,noSkip,G,no_iter) {
...
#X=as.matrix(as.numeric(headerLines[1,][,-seq(1,NoInfoColumns)]))
	...
	for (group_no in 1:G) {
		...
		for (c in 1:C) { # loop over 500 MCMC samples
			...
			#Sigma_propose=matrix(c(0.001,0,0,0.00001),2,2) # for gene
			Sigma_propose=matrix(c(0.03,0,0,0.0003),2,2) # for gene t5
			#Sigma_propose=matrix(c(0.3,0,0,0.03),2,2) # for transcript
			...
			ALPHA[group_no,c]=theta_est[1]
			BETA[group_no,c]=theta_est[2]
			ACC[group_no,c]=acc/no_iter
		}
		
		MU_groups[group_no]=mean(MU_group)
	...
	paramsFileName=paste(as.character(mcmc_filenames_in[1]),"_params",sep="")
	write.table(out_d,file=paramsFileName,quote=F,sep='\t',col.names=FALSE,row.names=FALSE)
	#smoothed_alpha=smooth(MU_groups,mean_alpha,0.4,'loess')
	#smoothed_beta=smooth(MU_groups,mean_beta,0.4,'loess')
